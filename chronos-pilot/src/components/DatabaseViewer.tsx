...
export default function DatabaseViewer({ systemTime, refreshTrigger, onRefresh, events = [] }: DatabaseViewerProps) {
  const [selectedTable, setSelectedTable] = useState<string>('commitments');
  const [filterQuery, setFilterQuery] = useState('');
  const [loading, setLoading] = useState(false);

  // Group events by table representation
  ...
  events.forEach(event => {
    if (event.event_type === 'CommitmentDiscovered') {
      tableData.commitments.push({
        id: tableData.commitments.length + 1,
        commitment_id: event.payload.commitment_id,
        title: event.payload.content,
        type: event.payload.source,
        deadline: event.payload.inferred_due_at,
        confidence: event.payload.confidence,
        timestamp: event.timestamp
      });
    } else if (event.event_type === 'ExecutionPlanCreated') {
      tableData.project_actions.push({
        id: tableData.project_actions.length + 1,
        plan_id: event.payload.plan?.execution_plan_id,
        steps: event.payload.plan?.execution_steps,
        timestamp: event.timestamp
      });
    } else if (event.event_type === 'GitRepositoryDiscovered') {
      const name = event.payload.repository_path?.split(/[\\/]/).pop() || 'Workspace';
      tableData.projects.push({
        id: tableData.projects.length + 1,
        name,
        path: event.payload.repository_path,
        timestamp: event.timestamp
      });
      tableData.context_nodes.push({
        key: `REPO:${event.payload.repository_path}`,
        type: 'REPO',
        name,
        timestamp: event.timestamp
      });
    } else if (event.event_type === 'FileModified' || event.event_type === 'VSCodeActiveFileChanged') {
      ...
      tableData.context_events.push({
        id: tableData.context_events.length + 1,
        node: name,
        type: 'MODIFIED',
        timestamp: event.timestamp
      });
    } else if (event.event_type === 'BrowserUrlChanged' || event.event_type === 'BrowserTabActivated') {
      const url = event.payload.url || '';
      tableData.browser_sessions.push({
        url,
      ...
      if (url.includes('google.com/search')) {
        ...
        const queryText = queryMatch ? decodeURIComponent(queryMatch[1]) : '';
        tableData.search_queries.push({
          query: queryText,
          timestamp: event.timestamp
        });
      }
    } else if (event.event_type === 'DeadLetterQueueAppended') {
      tableData.dead_letter_queue.push(event.payload);
    }
  });
