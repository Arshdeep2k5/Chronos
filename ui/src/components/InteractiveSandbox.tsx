...
export default function InteractiveSandbox({
    ...
    try {
      const res = await fetch(`${API_BASE}/api/sandbox/simulate`, {
        ...
        body: JSON.stringify({
          ...
          payload: { filename: simulatedPdfName }
        })
      });
      const data = await res.json();
      if (data.success) {
        triggerToast(`Commitment discovered! ${payloadLabel(data.discovered_commitment)}`, 'success');
        onRefresh();
      }
    } catch (e) {
      console.error(e);
  ...
  const payloadLabel = (c: any) => {
    return `'${c.title}' extracted successfully (Confidence: ${(c.confidence_score * 100).toFixed(0)}%)`;
  };
    ...
    try {
      const res = await fetch(`${API_BASE}/api/sandbox/simulate`, {
        ...
        body: JSON.stringify({
          ...
          payload: { query: searchQueryInput }
        })
      });
      const data = await res.json();
      if (data.success) {
        triggerToast(`Telemetry intercepted: "${searchQueryInput}" mapped to search graph`, 'info');
        setSearchQueryInput('');
        onRefresh();
      }
    } catch (e) {
  ...
  const handleCheckpointSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        triggerToast('Human ground-truth checkpoint submitted. Project weights re-indexed!', 'success');
        setAccomplished('');
        setBlocked('');
        setNextSteps('');
        onRefresh();
  ...
  const handleSystemPurge = async () => {
    if (confirm('Are you sure you want to purge system logs? This will reset all telemetry. (Privacy sovereignty demo)')) {
      try {
        const res = await fetch(`${API_BASE}/api/database/purge`, { method: 'POST' });
        const data = await res.json();
        if (data.success) {
          triggerToast('Privacy sovereignty: Entire system database cleared and reset locally!', 'warn');
          onRefresh();
        }
      } catch (e) {
        console.error(e);
