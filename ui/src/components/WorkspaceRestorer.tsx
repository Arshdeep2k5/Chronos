...
export default function WorkspaceRestorer({ projects }: WorkspaceRestorerProps) {
  ...
  const handleStartWorking = async (projectId: number) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/workspace/restore`, {
        method: 'POST',
      ...
      if (data.success) {
        setWhyStopped(data.why_stopped_narrative);
        setRestoredState(data.snapshot);
      }
    } catch (e) {
      console.error(e);
    } finally {
